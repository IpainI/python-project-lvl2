...
def generate_new_tree(tree, count=1):
    new_tree = []
    spaces = SPACES * count
    for key, value in tree.items():
        ...
        if item_type == 'CHILD':
            ...
            new_tree.extend([
                '{}+ {}: {}'.format(spaces, key, get_value(value.get('d1_value'))),
                '{}- {}: {}'.format(spaces, key, get_value(value.get('d2_value')))
            ])
        if item_type == 'REMOVED':
            new_tree.append('{}- {}: {}'.format(spaces, key, get_value(item_value, count)))
        if item_type == 'ADDED':
            new_tree.append('{}+ {}: {}'.format(spaces, key, get_value(item_value, count)))
